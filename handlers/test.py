...
async def cmd_test(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "Я предложу вам одно задание, которое поможет определить ваш уровень владения польским. "
...
async def determine_level(text):
    ...
    prompt = (
        "Как опытный преподаватель польского языка, определи уровень владения языком по шкале CEFR (A0, A1, A2, B1, B2) "
        ...
        "Верни только код уровня (A0, A1, A2, B1, B2) без пояснений."
    )
    
    try:
        result = await get_grok_response(prompt, TEST_SYSTEM_PROMPT)
        # Извлекаем уровень из ответа
        level_match = re.search(r'(A[0-2]|B1)', result)
        if level_match:
